...
def check_duplicate(r, target_ids):
    ...
    for uid in target_ids:
        if r.check_set_exists(REDIS_DUPLICATE_SET, uid):
            pass
        else:
            result.append(uid)
            r.push_set_data(REDIS_DUPLICATE_SET, uid)
...
def save_extract_ids(r, fids):
    ''' '''
    for uid in fids:
        r.push_list_data(REDIS_EXTRACT_QUEUE, uid)
        if r.check_set_exists(REDIS_TASK_DUPLICATE_SET, uid):
            pass
        else:
            r.push_list_data(REDIS_TASK_QUEUE, uid, direct='right')
